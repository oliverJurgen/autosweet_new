...
class VehicleInfoIconBlock extends React.Component<Props, any> {
  constructor(props: Props) {
    super(props);
  }

  renderCondition(condition: number) {
    if (condition === 1) return <div>Used</div>;
    if (condition === 2) return <div>New</div>;
  }

  render() {
    const { vehicleModel } = this.props;

    console.log({ condition: vehicleModel.condition });
    return (
            ...
            <div className={style.InformationBlockItem}>
              ...
              <p>
                ...
                <span className={style.gray}>
                  {this.renderCondition(vehicleModel?.condition)}
                  {/* {
                    {
                      1: <div>Used</div>,
                      0: <div>New</div>,
...
export default VehicleInfoIconBlock;
